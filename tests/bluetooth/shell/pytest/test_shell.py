...
def get_addr_with_name(lines: list[str], name: str):
    dut_adv_line = ''
...
def test_ble_connection(dut: DeviceAdapter, shell: Shell, harness_shells: list[Shell]) -> None:
    ...
    dut.readlines_until(regex=r'Bluetooth initialized', timeout=3)
    ...
    lines = dut.readlines_until(regex=rf'{adv_device_name}.*\r?\n?$', timeout=5)
    address, address_type = get_addr_with_name(lines, adv_device_name)
    ...
    dut.readlines_until(regex=rf'Connected: {address} \({address_type}\)', timeout=5)
    time.sleep(3)
