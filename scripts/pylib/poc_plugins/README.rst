...
- *Use ``harness_devices`` or ``harness_shells``*: Depend on either in your test functions to interact with the devices. ``harness_devices`` handles flashing and initialization.
- Each DeviceAdapter in ``harness_devices`` corresponds to a physical harness board.
