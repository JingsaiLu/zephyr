...
def harness_devices(
    ...
    for index, harness_build_dir in enumerate(harness_build_dirs):
        ...
        # logging all attributes of dut_device_config and harness_hw
        logger.info(f"dut_device_config attributes: {vars(dut_device_config)}")
        logger.info(f"harness_hw attributes: {vars(harness_hw)}")
