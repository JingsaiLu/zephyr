...
from typing import Generator, Type, Callable, List, Optional
...
def harness_devices(
    ...
    harness_devices_yaml: Optional[str] = None
    ...
    if not harness_device_hwm.duts or (len(harness_build_dirs) > len(harness_device_hwm.duts)):
        pytest.fail("Please check the harness_devices yaml, the number of devices and images do not match")
        return
...
def harness_shells(harness_devices: List[DeviceAdapter]) -> List[Shell]:
    ...
    for device_obj in harness_devices:
        shell = Shell(device_obj, timeout=20.0)
